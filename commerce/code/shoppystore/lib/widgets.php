...
function ya_widgets_init() {
	...
	register_widget('YA_Recommended_Products_Widget');
...
class YA_Posts_Widget extends YA_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'ya_posts', 'description' => __('SW Posts', 'shoppystore'));
		parent::__construct('ya_posts', __('SW Posts', 'shoppystore'), $widget_ops);
...
class YA_Category_List_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_category_list', 'description' =>__('SW Category List header widget', 'shoppystore'));
		parent::__construct('ya_category_list', __('SW List Category', 'shoppystore'), $widget_ops);
	}
}
class YA_Vertical_Megamenu_Widget extends YA_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'vertical_megamenu', 'description' =>__('SW Vertical Mega Menu Widget', 'shoppystore'));
		parent::__construct('vertical_megamenu', __('SW Vertical Mega Menu', 'shoppystore'), $widget_ops);
	}
}
...
class YA_Top_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_top', 'description' =>__('SW top header widget', 'shoppystore'));
		parent::__construct('ya_top', __('SW Top Widget', 'shoppystore'), $widget_ops);
	}
}
...
class YA_Top_Rate_Product extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_top_rate_product woocommerce widget_top_rated_products', 'description' =>__('SW Top Rated Product Widget', 'shoppystore'));
		parent::__construct('top_rated_product', __('SW Top Rated Product Widget', 'shoppystore'), $widget_ops);
	}
}
...
class YA_Best_Seller_Product extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_best_seller_product', 'description' =>__('SW best seller product woocommerce', 'shoppystore'));
		parent::__construct('ya_best_seller_product', __('SW best seller products', 'shoppystore'), $widget_ops);
	}
}
...
class YA_Related_Product_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_relate_product', 'description' =>__('SW related products woocommerce', 'shoppystore'));
		parent::__construct('ya_relate_product', __('SW related products', 'shoppystore'), $widget_ops);
	}
...
class YA_Recommended_Products_Widget extends YA_Widget{

	function __construct(){
		$widget_ops = array('classname' => 'ya_recommend_product', 'description' =>__('SW recommend products woocommerce', 'shoppystore'));
		parent::__construct('ya_recommend_product', __('SW recommend products', 'shoppystore'), $widget_ops);
	}
	public function widget( $args, $instance ) {
		extract($args);
...
class YA_Categories_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_categories', 'description' =>__('SW Categories', 'shoppystore'));
		parent::__construct('ya_categories', __('SW Catetories', 'shoppystore'), $widget_ops);
	}
}
...
class YA_Feature_Product_Widget extends YA_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_feature_product', 'description' =>__('SW feature product woocommerce', 'shoppystore'));
		parent::__construct('ya_feature_product', __('SW Feature Products', 'shoppystore'), $widget_ops);
	}
}
...
class YA_Social_Widget extends WP_Widget{
	...
	function __construct(){
		$widget_ops = array('classname' => 'ya_social', 'description' => __('SW Social Networks', 'shoppystore'));
		parent::__construct('ya_social', __('SW Social', 'shoppystore'), $widget_ops);
	...
	function form($instance){ 
		...
		?>
<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php esc_html_e('Title: ', 'shoppystore'); ?>
...
<?php 
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo __( 'Social', 'shoppystore' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'shoppystore')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'shoppystore')?> </label>
...
<?php
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'shoppystore' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'shoppystore'); ?>
		...
		icon, please choose class name in Font Awesome. This is required.', 'shoppystore' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'shoppystore')?> </label>
...
class YA_Cpanel extends WP_Widget{
	...
	public function __construct(){
		$widget_opts = array( 'classname' => 'cpanel', 'description' => __('Theme Options on Frontend', 'shoppystore') );
		parent::__construct('cpanel', __('YA cPanel', 'shoppystore'), $widget_opts);
	}
...
class YA_Widgets{
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){		
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo $widget->get_field_id('adoptions'); ?>"><?php esc_html_e('Advanced Options','shoppystore')?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo $widget->get_field_id('widget_style'); ?>"><?php esc_html_e( 'Widget Style', 'shoppystore' ) ?>
	...
	private function getDisplaySelectOptions(){
		if ( is_null($this->display_select_tpl) ){
			$this->display_select_options = $display_select_options = array(
				'all' => __('All', 'shoppystore'),
				'if_selected' => __('Only on selected', 'shoppystore'),
				'if_no_selected' => __('Except selected', 'shoppystore')
			);
			$html = '';
			$html .= '<p>';
			$html .= '<label for="id-display-select">'. esc_html__( 'Widget Display', 'shoppystore' ) .  '</label>';
	...
	private function getDisplayCheckboxUsers(){
		if ( is_null($this->display_checkbox_users_tpl) ){
			$this->display_checkbox_users = $display_checkbox_users = array(
				'login' => __('Show only for login users' , 'shoppystore'),
				'logout' => __('Show only for logout users' , 'shoppystore')
			);
	...
	private function getDisplayLanguage(){
		...
		$html = '';
		$html .= '<p>';
		$html .= '<label for="id-display-language">'. esc_html__( 'Language Display', 'shoppystore' ) .  '</label>';
	...
	private function getDisplayCheckboxGeneral(){
		if ( is_null($this->display_checkbox_general_tpl) ){
			$this->display_checkbox_general = $display_checkbox_general = array(
					'home' => __('Blog posts', 'shoppystore'),
					'archive' => __('Archive page', 'shoppystore'),
					'date' => __('Archive Date', 'shoppystore'),
					'category' => __('Category archive', 'shoppystore'),
					'tag' => __('Tag archive', 'shoppystore'),
					'author' => __('Author archive', 'shoppystore'),
					'search' => __('Search page', 'shoppystore'),
					'single' => __('Single post', 'shoppystore'),
					'page' => __('Page template', 'shoppystore'),
					'404' => __('Page 404', 'shoppystore')
				);
			$html = '';
	...
	private function render_wdisplayxx($widget_display){
		...
		$display_checkbox['general'] = array(
					'front_page' => __('Front page', 'shoppystore'),
					'home' => __('Blog posts', 'shoppystore'),
					'archive' => __('Archive page', 'shoppystore'),
					'category' => __('Category archive', 'shoppystore'),
					'tag' => __('Tag archive', 'shoppystore'),
					'author' => __('Author archive', 'shoppystore'),
					'search' => __('Search page', 'shoppystore'),
					'page' => __('Pages', 'shoppystore'),
					'single' => __('Single post page', 'shoppystore')
				);
		//Get pages
		$pages = get_pages();
		...
		<p>
			<label
				for="<?php echo $this->widget->get_field_id('display_select'); ?>"><?php esc_html_e( 'Widget Display', 'shoppystore' ) ?>
			</label>
