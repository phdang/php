(function($) {
	$(document).ready(function(){
		...
		$("img[src='']").attr("src", ya_upload.url);
		
		$('.ya-menu-upload').on('click', function() {
			header_clicked = true;
		...
		$('.ya-opts-upload-remove').click(function(){
			$relid = $(this).attr('rel-id');
			$('#'+$relid).val('');
			$(this).prev().fadeIn('fast');
			$(this).prev().prev().fadeOut('fast', function(){jQuery(this).attr("src", ya_upload.url);});
			$(this).fadeOut('slow');
		});
		
		$('.ya-menu-upload-remove').on('click', function(){
			$relid = $(this).attr('rel-id');
			$('#'+$relid).val('');
			$(this).prev().fadeIn('fast');
			$(this).prev().prev().fadeOut('fast', function(){$(this).attr("src", ya_menu_upload.url);});
			$(this).fadeOut('slow');
		});
	});
	function MenuClick(){
