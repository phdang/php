	...
	<div class="entry clearfix">
		<?php if( $format == '' || $format == 'image' ){ ?>
			...
			<div class="entry-content">				
				<div class="content-top">
					...
					<div class="entry-meta">
						...
						<span class="entry-comment">
							<a href="<?php comments_link(); ?>"><i class="fa fa-comments"></i><?php echo $post-> comment_count .  ( ($post-> comment_count) > 1 ? esc_html__('  Comments', 'shoppystore') : esc_html__('  Comment', 'shoppystore')); ?></a>
						</span>
					</div>
					<?php if(has_tag()) :?>
				...
				<div class="readmore"><a href="<?php echo get_permalink($post->ID)?>"><i class="fa fa-caret-right"></i><?php esc_html_e('Read More', 'shoppystore'); ?></a></div>
			</div>
		<?php } elseif( !$format == ''){?>
		<div class="wp-entry-thumb">	
			<?php if( $format == 'video' || $format == 'audio' ){ ?>	
				<?php echo ( $format == 'video' ) ? '<div class="video-wrapper">'. ya_get_entry_content_asset($post->ID) . '</div>' : ya_get_entry_content_asset($post->ID); ?>										
			<?php } ?>
