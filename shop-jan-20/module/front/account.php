...
<div class="account">
    ...
    <div class="account_table">
        ...
        <?php
			$sql = 'SELECT `id`, `create_at`, `name`, `status`, sum(`qty`*`price`) as `total` 
					FROM `nn_order` a,`nn_order_detail` b 
					WHERE b.`order_id`=a.`id` AND `user_id` = '.$userID .' 
					Group by `id`
					order by `id` DESC';
			$rs = mysqli_query($link,$sql);
		...
		?>
            <ul class="contable">
                <li class="order"><?=$r['id']?></li>
                <li class="date"><?=date('d/m/Y H:i',strtotime($r['create_at']))?></li>
                <li class="ship"><?=$r['name']?></li>
                <li class="ordertotal"><?=number_format($r['total'])?></li>
                <li class="status last">
				<?php
					echo $TrangThai[$r['status']];
				?></li>
                <li class="action last"><a href="?mod=view_order&id=<?=$r['id']?>" class="first">View</a>
                	<?php 
					$confirm_cancel = "confirm('Bạn có chắc muốn hủy') ";
					$cancel_content = '<a Onclick="return ' .$confirm_cancel . '" href="?mod=order_cancel&id=' . $r['id'] .'">Cancel</a>';
					
					if ($r['status']==0) echo $cancel_content; ?>
