<?php
	...
	$sql = "SELECT COUNT(*) AS `total` FROM `phd4shop_product` WHERE `active` = 1 AND `category_id` = {$cid} ORDER BY `id`";
	...
	$sql = "SELECT * FROM `phd4shop_product` WHERE `active` = 1 AND `category_id` = {$cid} ORDER BY `id` LIMIT {$pos},{$itemsPerPage}";
		$rs = mysqli_query($link,$sql);
		$r = mysqli_fetch_assoc($rs)
?>
<!DOCTYPE html>
        ...
        <div class="col2">
            ...
            <div class="sorting">
                ...
                <ul class="right">
                    ...
                    <a href="?cid=<?= $cid ?>&page=<?= max($page - $increment, 1) ?>" class="colr">&lt;</a> 
						...
						?>
                        <a class="<?= $i == $page ? "current" : "" ?>" href="?cid=<?= $cid ?>&page=<?= $i ?>"><?= $i ?></a> 
                        <?php
						$i++;
						}
						?>
                        <a href="?cid=<?= $cid ?>&page=<?= min($page + $increment, $totalPages) ?>" class="colr">&gt;</a> 
            ...
            <div class="listing">
				...
				if ($sort == 1) {
					...
					$sql = "SELECT * FROM `phd4shop_product` WHERE `active` = 1 AND `category_id` = {$cid} ORDER BY {$order} LIMIT {$pos},{$itemsPerPage}";
					$rs = mysqli_query($link,$sql);
					while ($r = mysqli_fetch_assoc($rs)) { ?>
                	<li>
                    	<a href="detail.html" class="thumb"><img src="images/sanpham/<?= $r['img_url'] ?>" alt="" ></a>
